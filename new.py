...
async def upload_to_s3_direct_async(content: bytes, file_name: str, bucket_name: str, s3_folder: str = "code"):
    def _upload():
...
async def execute_code(data: CodeExecutionRequest):
    ...
    try:
        ...
        # upload plot images generated by the sandbox
        ...
        # upload remaining /code artifacts (read as raw bytes to avoid corruption)
        ...
        for file in uploaded_files:
            ...
            try:
                raw_bytes = await asyncio.to_thread(sandbox.files.read, file.path, "bytes")
                filename = os.path.basename(file.path)
                url = await upload_to_s3_direct_async(bytes(raw_bytes), filename, bucket_name, "code")
                if url:
                    uploaded_file_urls.append(url)

                # delete artifact inside sandbox
                await asyncio.to_thread(
                    sandbox.run_code,
                    f"import os, pathlib; pathlib.Path('{file.path}').unlink(missing_ok=True)",
                )
            except Exception as e:
